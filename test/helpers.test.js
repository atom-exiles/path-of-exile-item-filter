  ...
  describe('FilterElements', function() {
    ...
    describe('isActionRule()', function() {
      it('returns true when given an Action rule', function() {
        assert(isActionRule(actionRule));
      });

      it('returns false when give a Filter rule', function() {
        assert(isActionRule(filterRule) === false);
      });

      it('returns false when given a Rule', function() {
        assert(isActionRule(rule) === false);
      });

      it('returns false when given any other filter element', function() {
        assert(isActionRule(empty) === false);
        assert(isActionRule(itemLevelRule) === false);
      });
    });
    ...
    describe('isEmpty()', function() {
      ...
      it('returns false when given any other filter element', function() {
        assert(isEmpty(lineComment) === false);
        assert(isEmpty(unknown) === false);
      });
    });

    describe('isLineComment()', function() {
      it('returns true when given a filter Rule', function() {
        assert(isLineComment(lineComment));
      });

      it('returns false when given any other filter element', function() {
        assert(isLineComment(empty) === false);
        assert(isLineComment(unknown) === false);
      });
    });
    ...
    describe('isUnknown()', function() {
      ...
      it('returns false when given any other filter element', function() {
        assert(isUnknown(empty) === false);
        assert(isUnknown(rule) === false);
      });
    });
  });
